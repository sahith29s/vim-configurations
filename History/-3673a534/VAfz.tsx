...
const Login = () => {
    ...
    // }
    useEffect(() => {
        // getmyData().then((value) => { setUserData(value) })
    }, [])

    const handleSubmit = async (e: any) => {
        e.preventDefault()
        let verfiyUser = await fetch('http://localhost:5000/login', {
            method: 'post',
        ...
        verfiyUser = await verfiyUser.json()
        ...
        ">
            <div className="bg-white p-8 rounded shadow-md w-96">
                ...
                <form onSubmit={handleSubmit}>
                    <div className="mb-4">
                        <label className="block text-sm font-medium mb-1" htmlFor="email">Password</label>
                        <input
                            type="text"
                            id="email"
                            name="email"

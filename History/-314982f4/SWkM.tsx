...
const Chat = () => {
    const [arrayOfMessageObj, setArrayOfMessageObjects] : any = useState([])
    ...
    const handleSend = () => {
        if (message) {
            ...
            let messageObject = { text: message, isUser: true }
                let emptyArray = [...arrayOfMessageObj]
                emptyArray.append(messageObjectFromServer)
                setArrayOfMessageObjects([emptyArray])
            // setArrayOfMessageObjects([...arrayOfMessageObj , messageObject])
            
            socket.emit("message" , messageObject)
            socket.on("messageToFrontend" ,  (messageObjectFromServer : any) =>{
                let emptyArray : any = [...arrayOfMessageObj]
                emptyArray.append(messageObjectFromServer)
                setArrayOfMessageObjects([emptyArray])
                console.log(arrayOfMessageObj)
                console.log(messageObjectFromServer)
            })
                ...
                <div className="bg-gradient-to-b from-blue-600 via-blue-500 to-blue-400 h-screen flex items-center flex-col">
                    ...
                    <div className='w-[80%]'>
                        ...
                        <div className="h-[80vh] flex-1 overflow-y-auto p-4">
                            {arrayOfMessageObj.map((message, index) => (
                                <div
                                    key={index}

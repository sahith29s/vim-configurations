...
router.post("/api/login", async (req, res) => {
    try {
        ...
        let  isPasswordMatch = await bcrypt.compare(req.body.password,signUpDataForChecking.password)

        if (signUpDataForChecking.email == req.body.email && signUpDataForChecking.username == req.body.username && isPasswordMatch) {

            let newLoginData = new loginData({
                username: req.body.username,
                email: req.body.email,
                password: signUpDataForChecking.password 
            })

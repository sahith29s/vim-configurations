            ...
            todos.map((item, index, arrary) => {
              ...
              return (
                <div key={item._id} onClick={() => { completeTodo(item._id) }} className={`todo cursor-pointer rounded-xl bg-slate-500 min-h- flex mb-5 items-center } `}>
                  ...
                  <div onClick={(e) => { deleteTodo(item._id, e) }} className="absolute z-10 cursor-pointer left-[81rem] bg-purple-800 w-6 text-center hover:w-6 hover:h-6 hover:bg-red-800 rounded-full">X</div>
                </div>
              )

    ...
    <>
      ...
      <div className="App text-white bg-black   min-h-[100vh] p-10 font-serif">
        ...
        <div>
          ...
          <button onClick={addTodo} className="ml-3 bg-orange-500 rounded-xl p-1 text-black "> Add Todo</button>
        </div>
        ...
        <div className="">
          {
            todos.map((item, index, arrary) => {
              ...
              return (
                <div key={item._id} onClick={() => { completeTodo(item._id) }} className={`todo cursor-pointer rounded-xl bg-green-500 min-h- flex mb-5 items-center } `}>
                  <div className={`w-5 h-5  rounded-full ml-2 ${item.complete ? "bg-white" : "bg-red-700"}`} ></div>

                  <div className={`ml-5 text-xl `}> <span className={item.complete ? "line-through" : ""}>{item.todo}</span> {item.complete ? <span className="">completed</span> : ""}</div>

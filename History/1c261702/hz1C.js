        ...
        <div className="">
          {
            todos.map((item, index, arrary) => {
              ...
              return (
                <div key={item._id} onClick={() => { completeTodo(item._id) }} className={
                  `todo cursor-pointer rounded-xl bg-slate-500 min-h- flex mb-5 items-center hover:opacity-[.8] `
                }>
                  
                  <div className={
                    `w-5 h-5 rounded-full ml-2 ${item.complete ? "bg-green-800" : "bg-slate-400"}`
                  } ></div>

                  <div className={`ml-5 text-xl `}>
                    <span className={item.complete ? "line-through" : ""}>{item.todo}</span>
                    {item.complete ? <span className="opacity-[.5]">completed</span> : ""}
                  </div>

                  <div onClick={(e) => { deleteTodo(item._id, e) }}
                    className="absolute z-10 cursor-pointer left-[81rem] bg-purple-800 w-6 text-center hover:w-7 hover:h-7 hover:bg-red-800 rounded-full">X</div>
                </div>
              )
